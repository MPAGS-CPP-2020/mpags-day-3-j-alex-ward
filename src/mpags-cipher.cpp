...
int main(int argc, char* argv[])
{
  ...
  if (cipherSettings.helpRequested) {
  ...
  if (cipherSettings.versionRequested) {
  ...
  if (!cipherSettings.inputFile.empty()) {

    // Open the file and check that we can read from it
    std::ifstream inputStream(cipherSettings.inputFile);
    if (!inputStream.good()) {
      std::cerr << "[error] failed to create istream on file '" << cipherSettings.inputFile << "'" << std::endl;
      return 1;
    }
  ...
  if ( ! cipherSettings.cipher_key.empty() ) {
    ...
    for ( const auto& elem : cipherSettings.cipher_key ) {
      if ( ! std::isdigit(elem) ) {
	std::cerr << "[error] cipher key must be an unsigned long integer for Caesar cipher,\n"
	          << "        the supplied key (" << cipherSettings.cipher_key << ") could not be successfully converted" << std::endl;
	return 1;
      }
    }
  ...
  if (!cipherSettings.outputFile.empty()) {
    ...
    std::ofstream outputStream(cipherSettings.outputFile);
    if (!outputStream.good()) {
      std::cerr << "[error] failed to create ostream on file '" << cipherSettings.outputFile << "'" << std::endl;
      return 1;
    }
